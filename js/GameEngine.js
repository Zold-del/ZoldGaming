...
class GameEngine {
    ...
    init(gameArea) {
        ...
        // Configure les contrôles tactiles pour mobile
        this.setupTouchControls(gameArea);
    ...
    resetGame() {
        ...
        // Affiche les contrôles tactiles si le jeu n'est pas en pause
        if (!this.paused) {
     ...
     */
    async saveHighScore() {
        console.log('Tentative de sauvegarde du score:', {
            gameOver: this.gameOver,
            score: this.score,
            lines: this.lines,
            level: this.level,
            isAuthenticated: window.apiService ? window.apiService.isAuthenticated() : false
        });
        ...
        if (this.gameOver && this.score > 0) {
            ...
            // Sauvegarde sur le serveur si l'utilisateur est connecté
            if (window.apiService && window.apiService.isAuthenticated()) {
                try {
                    ...
                    console.log('Envoi du score au serveur:', scoreData);
                    const result = await window.apiService.saveScore(scoreData);
                    console.log('Score sauvegardé sur le serveur avec succès:', result);
                } catch (error) {
                    console.error('Erreur lors de la sauvegarde du score sur le serveur:', error);
                }
            } else {
                console.log('Utilisateur non connecté, score non sauvegardé sur le serveur');
            }
        } else {
            console.log('Conditions non remplies pour la sauvegarde:', {
                gameOver: this.gameOver,
                score: this.score
            });
        }
    ...
    togglePause() {
        ...
        if (this.paused) {
            ...
            // Affiche les contrôles tactiles
            this.showTouchControls();
    ...
    destroy() {
        ...
        // Nettoie les intervalles de mouvement tactile
        ...
        // Supprime les contrôles tactiles du DOM
     ...
     * @param {HTMLElement} gameArea - L'élément DOM de la zone de jeu
     */
    setupTouchControls(gameArea) {
        // Vérifie si on est sur mobile
        const isMobile = window.innerWidth <= 768 || 'ontouchstart' in window;

        if (!isMobile) {
            return; // Pas de contrôles tactiles sur desktop
        }

        // Crée les contrôles tactiles
        this.createTouchControls(gameArea);

        // Écouteurs tactiles sur la grille
        this.setupTouchGridControls(gameArea);
    }

    /**
     * Crée les boutons de contrôle tactile
     * @param {HTMLElement} gameArea - L'élément DOM de la zone de jeu
     */
    createTouchControls(gameArea) {
        // Crée le conteneur des contrôles tactiles
        const touchControls = Utilities.createElement('div', { class: 'touch-controls' });

        // Groupe gauche (déplacement)
        const leftGroup = Utilities.createElement('div', { class: 'touch-control-group' });
        const leftBtn = Utilities.createElement('button', { class: 'touch-btn' }, '←');
        ...
        // Groupe central (rotation et hard drop)
        const centerGroup = Utilities.createElement('div', { class: 'touch-control-group' });
        const rotateBtn = Utilities.createElement('button', { class: 'touch-btn' }, '↻');
        rotateBtn.addEventListener('touchstart', (e) => {
            e.preventDefault();
            this.rotate();
        });

        const hardDropBtn = Utilities.createElement('button', { class: 'touch-btn hard-drop' }, '↓');
        ...
        centerGroup.appendChild(hardDropBtn);

        // Groupe droite (déplacement)
        const rightGroup = Utilities.createElement('div', { class: 'touch-control-group' });
        const rightBtn = Utilities.createElement('button', { class: 'touch-btn' }, '→');
        ...
        touchControls.appendChild(centerGroup);
        touchControls.appendChild(rightGroup);

        // Ajoute les contrôles au conteneur du jeu
        const gameContainer = gameArea.closest('.game-container');
        if (gameContainer) {
    ...
    /**
     * Configure les contrôles tactiles sur la grille
     * @param {HTMLElement} gameArea - L'élément DOM de la zone de jeu
     */
    setupTouchGridControls(gameArea) {
        let touchStartX = 0;
        let touchStartY = 0;
        ...
        gameArea.addEventListener('touchend', (e) => {
            ...
            // Swipe horizontal pour déplacer
            ...
            // Swipe vertical vers le bas pour soft drop
            else if (deltaY > 50 && Math.abs(deltaX) < 50) {
                this.softDrop();
            }
            // Tap rapide pour rotation
            else if (deltaTime < 300 && Math.abs(deltaX) < 30 && Math.abs(deltaY) < 30) {
                this.rotate();
            }
        });

        // Empêche le scroll par défaut sur mobile
    ...
    /**
     * Commence le déplacement vers la gauche (pour bouton tactile maintenu)
     */
    startMoveLeft() {
        if (this.gameOver || this.paused) return;
    ...
    /**
     * Commence le déplacement vers la droite (pour bouton tactile maintenu)
     */
    startMoveRight() {
        if (this.gameOver || this.paused) return;
    ...
    /**
     * Cache les contrôles tactiles (pendant les menus)
     */
    hideTouchControls() {

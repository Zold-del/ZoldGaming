...
router.post('/register', 
    ...
    async (req, res) => {
        console.log('Requête d\'inscription reçue:', { username: req.body.username, email: req.body.email });
        try {
            ...
            console.log('Données validées:', { username, email });
            ...
            if (existingUser) {
                console.log('Utilisateur existant trouvé:', existingUser.email === email ? 'email' : 'username');
                return res.status(400).json({
                    success: false,

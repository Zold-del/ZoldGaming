...
const utils = {
    ...
    async isTokenExpired(token) {
        try {
            ...
            // Considérer le token comme expiré s'il reste moins de 5 minutes
    ...
    // Rafraîchir le token en utilisant l'endpoint /auth/me
    async refreshToken() {
        try {
            const currentToken = localStorage.getItem('blockdrop_token') || localStorage.getItem('token');
            if (!currentToken) {
                throw new Error('No token to refresh');
            }

            // Faire un appel à /auth/me pour vérifier et potentiellement rafraîchir la session
            ...
            if (response.ok) {
                // Le token est encore valide, le retourner
                return currentToken;
    ...
    async apiCall(endpoint, options = {}) {
        ...
        let token = localStorage.getItem('blockdrop_token') || localStorage.getItem('token');
        
        // Vérifier si le token est expiré et essayer de le rafraîchir
        if (token && await utils.isTokenExpired(token)) {
            try {
                token = await utils.refreshToken();
            } catch (refreshError) {
                console.warn('Token refresh failed:', refreshError);
                // Si le refresh échoue, continuer avec le token actuel
                // L'API retournera une erreur 401 que nous gérerons
            }
        }
        ...
        if (!response.ok) {
            // Si c'est une erreur 401 (token expiré), essayer de rafraîchir le token
            if (response.status === 401 && data.message?.includes('expir')) {
                try {
...
async function loadScores(page = 1, mode = '') {
    console.log('Chargement des scores admin, page:', page, 'mode:', mode);
    const params = new URLSearchParams({ page, limit: 50, mode });
    const data = await utils.apiCall(`/admin/scores?${params}`);
    console.log('Données des scores reçues:', data);
